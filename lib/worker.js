...
Worker.prototype.runServer = function (config) {
    ...
    var errorMessage = function (res, message, code) {
        ...
        var ResponseTransform = function() {
            Transform.call(this);
        }
        util.inherits(ResponseTransform, Transform);
        ResponseTransform.prototype._transform = function (data, encoding, processed) {
            data = data.toString().replace(
                'This domain is not associated with an application',
                'This domain (' + res.req.headers.host + ') is not associated with an application'
            );
            this.push(data);
            processed();
        };
    ...
    var httpRequestHandler = function (req, res) {
        ...
        (function () {
            res.req = req;  // Error handlers have res but not req
